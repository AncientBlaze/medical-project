    ...
    <button onClick={onClick}
      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors border ${
        active
          ...
          : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-900/40 border-transparent'
      }`}>
      ...
      {badge != null && (
        <span className={`text-xs font-semibold px-2 py-0.5 rounded-md ${
          active
            ...
            : 'bg-slate-200 dark:bg-slate-900/40 text-slate-600 dark:text-slate-500'
        }`}>
          {badge}
        </span>
      )}
