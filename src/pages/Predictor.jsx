...
// Named thresholds — change in one place, reflected everywhere
const SAFE_THRESHOLD = 0.85; // rank ≤ cutoff * 0.85 → safe
const UNLIKELY_BUFFER = 0.10; // rank ≤ cutoff * 1.10 → include as unlikely
...
const EMPTY_FORM = {
  state: 'West Bengal', category: 'UR', rank: '',
  quota: 'state', round: 'round1', mgmtFilter: 'all',
};

// ── Session-storage helpers ──────────────────────────────────────────────────

const pendingStorage = {
  save: (d) => { try { sessionStorage.setItem('pendingPrediction', JSON.stringify(d)); } catch { } },
  load: () => { try { const r = sessionStorage.getItem('pendingPrediction'); return r ? JSON.parse(r) : null; } catch { return null; } },
  clear: () => { try { sessionStorage.removeItem('pendingPrediction'); } catch { } },
};

// ── Data helpers ──────────────────────────────────────────────────────────────

// Pre-group colleges by state at module load — O(1) lookup instead of O(n) filter per render
const COLLEGES_BY_STATE = allColleges.reduce((map, c) => {
...
function getCollegesForState(state) {
  return COLLEGES_BY_STATE.get(state) ?? [];
}

// WB state quota maps OBC → OBC-A
function getStateQuotaKey(category, isWB) {
  return isWB && category === 'OBC' ? 'OBCA' : category;
}
...
function getCutoff(college, quotaType, category, round, isWB) {
  ...
  if (quotaType === 'allIndia') {
    return cutoffs.allIndiaQuota?.[category]?.[round] ?? null;
  }
  if (quotaType === 'state') {
    const stateKey = getStateQuotaKey(category, isWB);
    return cutoffs.stateQuota?.[stateKey]?.[round] ?? null;
  }
...
function getEligibility(rank, cutoff) {
  ...
  if (rank <= cutoff * SAFE_THRESHOLD) return 'safe';
...
function predictColleges({ rank, category, quota, round, mgmtFilter, state }) {
  ...
  return pool
    .filter((c) => {
      ...
      return cutoff && rank <= cutoff * (1 + UNLIKELY_BUFFER);
    })
    .map((c) => {
      const _cutoff = getCutoff(c, quota, category, round, isWB);
      const _eligibility = getEligibility(rank, _cutoff);
      return { ...c, _cutoff, _eligibility };
    })
    .sort((a, b) => (a._cutoff ?? Infinity) - (b._cutoff ?? Infinity));
}

// Persist a prediction to the backend — extracted to avoid duplication
async function savePredictionToApi(user, predictionObj) {
  try {
    await api.post('/predictions', {
      ...predictionObj,
      phone: user.phone,
      matchedColleges: predictionObj.matchedColleges.map((c) => c.name),
    });
  } catch (err) {
...
const InfoTooltip = ({ text }) => {
  ...
  return (
    <span className="relative inline-block">
      <Info
        className="w-3.5 h-3.5 text-slate-400 cursor-pointer hover:text-[#2C2E69] dark:hover:text-slate-300 transition-colors"
        // Mouse
        onMouseEnter={open} onMouseLeave={close}
        // Keyboard / touch
        tabIndex={0} onFocus={open} onBlur={close}
        role="button" aria-label="More information"
      />
      {show && (
...
const SelectField = ({ label, name, value, onChange, options, icon: Icon, tooltip }) => (
  <div>
    <label
      htmlFor={name}
      className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider mb-2 text-slate-500 dark:text-white/40"
    >
      {label} {tooltip && <InfoTooltip text={tooltip} />}
    </label>
    <div className="relative">
      {Icon && (
        <Icon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none text-slate-400 dark:text-[#2C2E69] z-10" />
      )}
      <select
...
const CollegeCard = ({ college, index, quota, round, category, isWB }) => {
  const meta = ELIGIBILITY_META[college._eligibility] ?? ELIGIBILITY_META.unlikely;
  const isGovt = college.management !== 'Private';

  const feeShow =
    quota === 'privateManagement' ? college.fees?.managementQuota
      : quota === 'state' ? college.fees?.stateQuota
        : null;

  // Resolve the correct state-quota key once for the round grid
  const stateQuotaKey = getStateQuotaKey(category, isWB);
    ...
    <div className={`group flex flex-col rounded-2xl overflow-hidden transition-all duration-200 border bg-white dark:bg-slate-900/50 hover:shadow-lg ${meta.card}`}>
      <div className="px-5 pt-5 pb-4 border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center justify-between gap-2 mb-2">
          {/* Left - Icon */}
          <span className="flex items-center justify-center w-7 h-7 rounded-lg text-xs font-bold bg-[#F9B406]/15 dark:bg-teal-500/15 text-[#c8920a] dark:text-teal-400">
            {index + 1}
          </span>
        ...
        <div className="flex items-center gap-1.5 mt-2 flex-wrap">
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${isGovt
          ...
          {college.established && (
            <span className="text-[10px] text-slate-400 dark:text-[#2C2E69]">Est. {college.established}</span>
          )}
        </div>
      </div>

      <div className="flex flex-col flex-1 p-5 gap-3">
        {/* Round-by-round cutoff grid — correctly reflects quota + category */}
        <div className="grid grid-cols-2 gap-2">
          {Object.entries(ROUND_LABELS).map(([r, label]) => {
            let val = null;
            if (quota === 'allIndia') {
              val = college.cutoffs?.allIndiaQuota?.[category]?.[r];
            } else if (quota === 'state') {
              val = college.cutoffs?.stateQuota?.[stateQuotaKey]?.[r];
            } else {
            ...
            return (
              <div
                key={r}
                className={`flex flex-col gap-0.5 px-2.5 py-2 rounded-lg border transition-colors ${isActive
                  ? 'bg-[#F9B406]/8 dark:bg-teal-500/10 border-[#F9B406]/30 dark:border-teal-500/30'
                  : val
                    ? 'bg-slate-50 dark:bg-slate-800/60 border-transparent'
                    : 'bg-slate-50/50 dark:bg-slate-800/20 border-transparent opacity-40'
                  }`}
              >
                <span className={`text-[10px] font-semibold uppercase tracking-wider ${isActive ? 'text-[#c8920a] dark:text-teal-400' : 'text-slate-400 dark:text-[#2C2E69]'}`}>
                  {label}
                </span>
...
const LoginGate = ({ pendingPrediction }) => {
  ...
  const goToAuth = (path) => {
    pendingStorage.save(pendingPrediction);
...
const SummaryBar = ({ predictions }) => {
  ...
  const total = predictions.matchedColleges.length;
  ...
  const chips = [
    ...
    { label: 'Total', val: `${total} college${total !== 1 ? 's' : ''}` },
  ];
...
// Simple skeleton placeholder shown during the prediction loading state
const ResultsSkeleton = () => (
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-6">
    {Array.from({ length: 6 }).map((_, i) => (
      <div key={i} className="rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900/50 h-64 animate-pulse">
        <div className="h-24 bg-slate-100 dark:bg-slate-800 rounded-t-2xl" />
...
const Predictor = () => {
  ...
  const [inputKey, setInputKey] = useState(0); // forces rank input remount on reset
  ...
  // College count for the currently selected state (memoised)
  const stateCollegeCount = useMemo(
    () => getCollegesForState(formData.state).length,
    [formData.state],
  );
  ...
  useEffect(() => {
    if (!user) return;
    const restored = pendingStorage.load();
    if (!restored) return;
    ...
    setShowResults(true);
    savePredictionToApi(user, restored);
  }, [user]);
  ...
  // Clear the form error only on blur so we don't wipe it mid-keystroke
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    startTransition(() => {
      ...
      if (user) savePredictionToApi(user, stateObj);
    });
  }, [formData, user]);
  ...
  const resetForm = useCallback(() => {
    ...
    setFormError('');
    setInputKey((k) => k + 1); // remounts the rank <input> to clear cached browser value
    pendingStorage.clear();
  }, []);
  ...
  //               <label htmlFor="state" className="block text-xs font-semibold uppercase tracking-wider mb-2 text-slate-500 dark:text-white/40">
  //                 State
  //               </label>
  //               <div className="relative">
  ...
  //             {/* Rank */}
  //             <div>
  //               <label htmlFor="rank" className="block text-xs font-semibold uppercase tracking-wider mb-2 text-slate-500 dark:text-white/40">
  //                 NEET Rank (AIR)
  //               </label>
  //               <input
  //                 key={inputKey}
  //                 id="rank" type="number" name="rank" value={formData.rank}
  //                 onChange={handleChange} onBlur={handleRankBlur}
  //                 min="1" placeholder="e.g. 25000" required
  ...
  //               onChange={handleChange} options={CATEGORIES} icon={GraduationCap}
  //               tooltip={isWB
  //                 ? 'WB State Quota maps OBC → OBC-A. OBC-B students may qualify at a slightly lower cutoff.'
  //                 : 'Select your NEET category.'
  //               }
  //             />

  //             <SelectField
  //               label="Quota Type" name="quota" value={formData.quota}
  //               onChange={handleChange} options={QUOTA_TYPES} icon={Globe}
  ...
  //               AIQ is only available at Govt / Central Govt colleges — private colleges are automatically excluded.
  //             </div>
  //           )}
  ...
  //               <Info className="w-3.5 h-3.5 shrink-0 mt-0.5" />
  //               Private Management cutoffs are not category-specific — the same cutoff applies regardless of category.
  //             </div>
  //           )}
  ...
  //               <Info className="w-3.5 h-3.5 shrink-0 mt-0.5" />
  //               WB State Quota uses OBC-A (stricter cutoff). OBC-B candidates may qualify at slightly higher ranks — verify with WBMCC.
  //             </div>
  //           )}
  //           {!isWB && (
  ...
  //               <Info className="w-3.5 h-3.5 shrink-0 mt-0.5" />
  //               Data for {formData.state} may have fewer rounds or categories than WB. Where data is unavailable, cells show —. Always verify with official state counselling authority.
  //             </div>
  //           )}
  ...
  //         {showResults && predictions && !loading && (
  //           <>
  //             <div ref={summaryRef}>
  ...
  //                 <>
  //                   {/* aria-live so screen readers announce the count after submit */}
  //                   <div
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 bg-[#fffdf7] dark:bg-slate-950 text-[#2D409C] dark:text-white">
      ...
      <div className="relative border-b border-amber-200 dark:border-slate-800 bg-linear-to-br from-amber-50 to-amber-100 dark:from-slate-900/60 dark:to-slate-950 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 lg:py-16">
          <div className="text-center mb-4 sm:mb-6">
            ...
            <h1 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight mb-3 sm:mb-4 text-[#2D409C] dark:text-white">
              Find Your{' '}
              <span className="text-transparent bg-clip-text bg-linear-to-r
                from-[#F9B406] to-[#F9B406]/60
                dark:from-teal-400 dark:to-teal-400/60"
            ...
            <p className="hidden sm:block text-sm sm:text-base max-w-xl mx-auto text-slate-600 dark:text-slate-400">
              Enter your rank, select your state and category to get matched against real 2026 cutoff data across 20 states.
            </p>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10 lg:py-12">
        ...
        <div className="rounded-2xl p-4 sm:p-6 lg:p-8 mb-5 sm:mb-8 relative overflow-hidden border bg-white dark:bg-slate-900/40 border-slate-200 dark:border-slate-700/60 shadow-sm">
          <div className="absolute top-0 right-0 w-72 h-72 rounded-full blur-3xl opacity-[0.04] pointer-events-none bg-[#F9B406] dark:bg-teal-400" />
          ...
          <div className="flex items-center justify-between gap-2 mb-5">
            ...
            {formData.state && (
              <span className="hidden sm:block text-xs text-slate-400 dark:text-slate-600">
                {stateCollegeCount} colleges in {formData.state}
              </span>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {formData.quota === 'allIndia' && (
              <div className="flex items-start gap-2 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 rounded-xl px-3 sm:px-4 py-3 text-xs text-blue-700 dark:text-blue-300 mb-4 sm:mb-5">
                <Info className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                AIQ is only available at Govt / Central Govt colleges — private colleges are automatically excluded.
              </div>
            )}
            {formData.quota === 'privateManagement' && (
              <div className="flex items-start gap-2 bg-purple-50 dark:bg-purple-500/10 border border-purple-200 dark:border-purple-500/20 rounded-xl px-3 sm:px-4 py-3 text-xs text-purple-700 dark:text-purple-300 mb-4 sm:mb-5">
                <Info className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                Private Management cutoffs are not category-specific — the same cutoff applies regardless of category.
              </div>
            )}
            {formData.category === 'OBC' && formData.quota === 'state' && isWB && (
              <div className="flex items-start gap-2 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl px-3 sm:px-4 py-3 text-xs text-amber-700 dark:text-amber-300 mb-4 sm:mb-5">
                <Info className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                WB State Quota uses OBC-A (stricter cutoff). OBC-B candidates may qualify at slightly higher ranks — verify with WBMCC.
              </div>
            )}
            {!isWB && (
              <div className="flex items-start gap-2 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/60 rounded-xl px-3 sm:px-4 py-3 text-xs text-[#2C2E69] dark:text-slate-400 mb-4 sm:mb-5">
                <Info className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                Data for {formData.state} may have fewer rounds or categories than WB. Where data is unavailable, cells show —. Always verify with official state counselling authority.
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <button
                type="submit" disabled={loading}
                className="flex items-center justify-center gap-2 w-full sm:w-auto px-7 py-3 rounded-xl font-semibold text-sm disabled:opacity-60 transition-all shadow-md bg-[#F9B406] dark:bg-teal-400 hover:bg-[#e0a205] dark:hover:bg-teal-300 text-slate-950 shadow-[#F9B406]/25 dark:shadow-teal-400/20"
              >
                {loading
                  ? <><span className="w-4 h-4 border-2 rounded-full animate-spin border-slate-900/30 border-t-slate-900" />Predicting…</>
                  : <><Search className="w-4 h-4" /> Predict Colleges</>
              ...
              {showResults && (
                <button
                  type="button" onClick={resetForm}
                  className="flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-3 rounded-xl font-semibold text-sm border transition-colors text-[#2C2E69] dark:text-slate-300 bg-slate-100 dark:bg-slate-900/40 hover:bg-slate-200 dark:hover:bg-slate-800/60 border-slate-200 dark:border-slate-800"
                >
                  <RotateCcw className="w-4 h-4" /> Reset
                </button>
              )}
            ...
            <>
              ...
              {user ? (
                predictions.matchedColleges.length > 0 ? (
                  <>
                    <div
                      className="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-5"
                      aria-live="polite"
                    >
                      <h3 className="text-base sm:text-lg font-bold text-[#2C2E69] dark:text-white">
                        {predictions.matchedColleges.length} Matched College{predictions.matchedColleges.length !== 1 ? 's' : ''}
                      </h3>
                      <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-500">
                        <TrendingDown className="w-3.5 h-3.5" /><span>Sorted by cutoff rank</span>
                    ...
                    <div className="flex items-start gap-2.5 p-3 sm:p-4 rounded-xl border mb-5 sm:mb-6 text-xs bg-blue-50 dark:bg-blue-500/5 border-blue-200 dark:border-blue-500/20 text-blue-700 dark:text-blue-400">
                      <Info className="w-3.5 h-3.5 mt-0.5 shrink-0" />
                      Predictions are based on NEET 2026 counselling cutoff data. Actual admission depends on seat availability, state-specific rules, and current-year counselling trends. Always verify with official MCC / state counselling portals.
                    </div>
        ...
        {!showResults && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-3 sm:mt-4">
            {FEATURES.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="flex gap-3 sm:gap-4 p-4 sm:p-5 rounded-2xl border transition-colors bg-white dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 hover:border-amber-200 dark:hover:border-slate-700 shadow-sm">
                <div className="flex items-center justify-center w-9 h-9 rounded-xl border shrink-0 bg-amber-50 dark:bg-teal-500/10 border-amber-200 dark:border-teal-500/20">
                  <Icon className="w-4 h-4 text-[#F9B406] dark:text-teal-400" />
                </div>
                <div>
